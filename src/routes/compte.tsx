...
function ProfileTab({ userId, email }: { userId: string; email: string }) {
  ...
  const save = useMutation({
    ...
    onSuccess: () => { toast.success("Profil mis à jour"); qc.invalidateQueries({ queryKey: ["profile", userId] }); },
...
function AddressesTab({ userId }: { userId: string }) {
  ...
  const add = useMutation({
    ...
    onSuccess: () => { toast.success("Adresse ajoutée"); setForm(empty); setAdding(false); qc.invalidateQueries({ queryKey: ["addresses", userId] }); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const del = useMutation({
    ...
    onSuccess: () => { toast.success("Adresse supprimée"); qc.invalidateQueries({ queryKey: ["addresses", userId] }); },
  });
  ...
  return (
    <div className="space-y-4">
        ...
        {addresses.map((a) => (
          <div key={a.id} className="border border-border bg-card p-5 text-sm">
            ...
            <div className="mt-3 flex items-center gap-3">
              {a.is_default ? (
                <span className="text-xs uppercase tracking-widest text-copper">Adresse par défaut</span>
              ) : (
      ...
      {!adding ? (
        <Button variant="outline" onClick={() => setAdding(true)} className="gap-2"><Plus className="h-4 w-4" /> Ajouter une adresse</Button>
      ) : (
        <form onSubmit={(e) => { e.preventDefault(); add.mutate(); }} className="grid gap-4 border border-border bg-card p-6">
          <div className="grid gap-2"><Label>Nom complet *</Label><Input required value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} /></div>
          <div className="grid gap-2"><Label>Adresse ligne 1 *</Label><Input required value={form.line1} onChange={(e) => setForm({ ...form, line1: e.target.value })} /></div>
          <div className="grid gap-2"><Label>Adresse ligne 2</Label><Input value={form.line2} onChange={(e) => setForm({ ...form, line2: e.target.value })} /></div>
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="grid gap-2"><Label>Code postal *</Label><Input required value={form.postal_code} onChange={(e) => setForm({ ...form, postal_code: e.target.value })} /></div>
            <div className="grid gap-2 sm:col-span-2"><Label>Ville *</Label><Input required value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} /></div>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-2"><Label>Pays</Label><Input value={form.country} onChange={(e) => setForm({ ...form, country: e.target.value })} /></div>
          ...
          <div className="flex gap-2">
            <Button type="submit" disabled={add.isPending}>{add.isPending ? "Ajout…" : "Ajouter"}</Button>
            <Button type="button" variant="outline" onClick={() => { setAdding(false); setForm(empty); }}>Annuler</Button>
          </div>
        </form>
      )}
    </div>
...
function FavoritesTab({ userId }: { userId: string }) {
  ...
  if (!favs.length) return (
    <div className="border border-dashed border-border bg-card p-10 text-center text-sm text-muted-foreground">
      Aucun favori. <Link to="/boutique" className="text-copper hover:underline">Découvrir la boutique</Link>
    </div>
  );
    ...
    <div className="overflow-x-auto border border-border bg-card">
      <table className="w-full text-sm">
        <thead className="bg-secondary/50 text-left text-xs uppercase tracking-widest">
          <tr><th className="px-4 py-3">Commande</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Statut</th><th className="px-4 py-3 text-right">Total</th></tr>
        </thead>
        <tbody>
          {orders.map((o) => (
